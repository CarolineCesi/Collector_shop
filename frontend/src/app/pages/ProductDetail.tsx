...
import { ChevronRight, ShieldCheck, Heart, Share2, Star, CheckCircle2, Lock, ArrowLeft } from "lucide-react";
...
export function ProductDetail() {
  ...
  const handleFavoriteClick = () => {
    ...
    // Handle actual favorite functionality here
  ...
  if (loading) return <div className="p-24 text-stone-100 text-center text-xl">Loading product details...</div>;
  if (error || !item) return <div className="p-24 text-red-500 text-center text-xl">Error loading product or product not found.</div>;
          ...
          <div className="grid grid-cols-3 gap-4">
            {item.images.map((img: string, idx: number) => (
              <button
                ...
                className={`aspect-square bg-cyan-900/40 rounded-xl overflow-hidden border-2 transition-all p-2
                  ${activeImage === idx ? 'border-amber-400' : 'border-cyan-800/50 hover:border-cyan-700'}`}
              >
                <ImageWithFallback
                  src={img}
        ...
        <div className="flex flex-col">
          <div className="mb-6">
            <div className="flex justify-between items-start gap-4 mb-4">
              <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-stone-100 leading-tight">
                {item.title}
              </h1>
              <div className="flex gap-2 shrink-0">
                <button
                  onClick={handleFavoriteClick}
                  className="w-10 h-10 rounded-full border border-cyan-800 flex items-center justify-center text-cyan-400 hover:text-amber-500 hover:border-amber-500 hover:bg-amber-500/10 transition-all"
                >
                  <Heart size={20} />
                </button>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm mb-6">
              <span className="bg-cyan-900/60 border border-cyan-700 text-stone-300 px-3 py-1 rounded-full font-medium">
                {item.condition}
              </span>
              <span className="flex items-center gap-1.5 text-amber-400 bg-amber-400/10 px-3 py-1 rounded-full font-medium border border-amber-400/20">
                <ShieldCheck size={16} /> Authenticity Guaranteed
              </span>
            </div>

            <div className="text-4xl md:text-5xl font-bold text-stone-100 font-mono tracking-tighter mb-8">
              {item.price}
            </div>
          </div>
          ...
          <div className="space-y-4 mb-10">
            <button className="w-full bg-amber-400 hover:bg-amber-300 text-cyan-950 py-4 px-8 rounded-xl font-bold text-lg transition-transform hover:scale-[1.02] flex items-center justify-center gap-2 shadow-lg shadow-amber-400/20">
              <Lock size={20} /> Buy Now (Secure Payment)
            </button>
          ...
          <div className="bg-cyan-900/30 border border-cyan-800/50 rounded-2xl p-6 mb-10 space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-stone-100 mb-1">Buyer Protection</h4>
                <p className="text-sm text-stone-400 leading-relaxed">
                  Get the item you ordered or your money back. We hold funds until you confirm receipt.
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-stone-100 mb-1">Secure Transaction via Stripe</h4>
                <p className="text-sm text-stone-400 leading-relaxed">
          ...
          <div className="border-t border-cyan-900 pt-8 mb-10">
            <h3 className="text-lg font-bold text-stone-100 mb-4">About the Seller</h3>
            <div className="flex items-center justify-between bg-cyan-900/30 border border-cyan-800/50 rounded-2xl p-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-cyan-400 mt-1">
                    <span className="flex items-center gap-1">
                      ...
                      <span className="font-semibold text-stone-200">{item.seller.rating}</span>
                    </span>
                    <span>•</span>
              ...
              <button className="bg-cyan-800 hover:bg-cyan-700 text-stone-100 px-4 py-2 rounded-lg text-sm font-semibold transition-colors border border-cyan-700">
                Follow
              </button>
            </div>
          </div>
          ...
          <div className="border-t border-cyan-900 pt-8">
            <h3 className="text-lg font-bold text-stone-100 mb-4">Item Description & History</h3>
